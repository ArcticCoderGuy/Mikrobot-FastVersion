...
//--- Global variables
datetime lastCheck;
string currentBuild = "20250103-008F";
...
{
    Print("======================================================================");
    Print("BUILD: 20250103-008F - MIKROBOT_FASTVERSION.md COMPLIANT");
...
{
    MqlRates rates[];
    if(CopyRates(symbol, PERIOD_M5, 0, 15, rates) < 15)
        return false;
        
    // IMPROVED M5 BOS detection: Find structural highs/lows over multiple candles
    double currentHigh = rates[14].high;
    double currentLow = rates[14].low;
    
    // Find the highest high and lowest low over the previous 10 candles (structure period)
    double structuralHigh = rates[4].high;  // Start from older candle
    double structuralLow = rates[4].low;
    
    for(int i = 5; i < 14; i++)  // Look at candles 5-13 (avoid current candle)
    {
        if(rates[i].high > structuralHigh)
            structuralHigh = rates[i].high;
        if(rates[i].low < structuralLow)
            structuralLow = rates[i].low;
    }
    
    // BOS occurs when current candle breaks above/below structural level
    bool bullishBOS = (currentHigh > structuralHigh);
    bool bearishBOS = (currentLow < structuralLow);
    
    if(bullishBOS || bearishBOS)
    {
        m5_bos_time = rates[14].time;
        m5_bos_price = bullishBOS ? structuralHigh : structuralLow;  // Use structural level, not current price
        m5_bos_direction = bullishBOS ? "BULL" : "BEAR";
        
        if(DebugMode)
        {
            Print("M5 BOS DETECTED: ", symbol, " - ", m5_bos_direction);
            Print("  Structural Level: ", bullishBOS ? structuralHigh : structuralLow);
            Print("  Current Break: ", bullishBOS ? currentHigh : currentLow);
            Print("  BOS Price (structural): ", m5_bos_price);
...
bool DetectM1Break(string symbol)
{
    ...
    // M1 break detection: Must break above/below M5 BOS level
    ...
    // For bull: M1 candle must break above M5 BOS level
    if(m5_bos_direction == "BULL" && currentHigh > m5_bos_price)
        m1Break = true;
    // For bear: M1 candle must break below M5 BOS level  
    else if(m5_bos_direction == "BEAR" && currentLow < m5_bos_price)
        m1Break = true;
        
    if(m1Break)
    {
        m1_break_time = rates[4].time;
        // Use actual M1 break price where the break occurred
